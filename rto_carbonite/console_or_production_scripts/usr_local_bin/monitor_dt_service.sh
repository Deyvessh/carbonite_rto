...
LOG_FILE="/var/log/ngdrs-ag_rto.log"
...
calculate_rto() {
    ...
    if [ -n "$stop_time" ] && [ -n "$start_time" ]; then
        ...
        log_event "RTO calculated for ngdrs-ag: $rto seconds."
    else
        log_event "No RTO calculation for ngdrs-ag: stop_time or start_time is missing."
    fi
}
